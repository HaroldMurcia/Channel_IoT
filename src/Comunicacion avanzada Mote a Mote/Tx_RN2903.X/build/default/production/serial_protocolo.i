...
int Len(char *Data);
# 7 "serial_protocolo.c" 2
# 24 "serial_protocolo.c"
int Len(char *Data)
{
    int L = 0;
    while(Data[L]!='\0')
...
void DATAtoHEXA(uint64_t datahex, int Type)
{
# 56 "serial_protocolo.c"
    int i = 0,k = 0,condition=0,leng = 0,residue = 0;
    uint64_t result;
...
# 108 "serial_protocolo.c"
    do
    {
        residue = datahex%16;
        result = datahex/16;
        int_to_hex[i] = residue;
        datahex = result;
        i++;
    } while (result >15);
...
uint8_t CharacterHex_Decimal(char character )
{
# 172 "serial_protocolo.c"
    if((0 ? isdigit(character) : ((unsigned)(character)-'0') < 10))
    {
        return character -'0';
    }
    else
